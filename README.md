...
모델은 미리 정하지 않고 규칙 기반 단일 문턱부터 결정트리·로지스틱 회귀·랜덤포레스트·부스팅까지 피험자 단위 교차검증(LOSO)으로 비교했습니다. 특징 후보 14개 중 새 사람에게 통한 것은 `mean_rb` 하나였고, 특징이 하나라 로지스틱 회귀는 곧 문턱 규칙이 됩니다. 칩에 들어가는 파라미터는 문턱 T 하나(소수 10비트 고정소수점 `T_FIX` = 1086, 11비트 정수, 헛경보 시간당 4회 동작점)입니다.

## 결과 요약

| 항목 | 값 |
|---|---|
| 판정 성능, 처음 보는 사람 (LOSO, 헛경보 4회/h 동작점) | 사건 민감도 0.413 [0.28–0.53], 깊은 사건(피로2+) 0.633 [0.48–0.80] |
| 판정 성능, 칩 재현 (T_FIX 1086 하나, 창 품질·기준선 규칙 포함, 50명) | 사건 민감도 0.379, 깊은 사건 0.567, 헛경보 덩어리 3.94회/h |
| 실제로 울리는 경보 (심박 경보 30초 간격, 50명) | 깨어 있을 때 시간당 26번, 피로1 이상일 때 45번. 울린 경보 중 실제 피로1 이상 34% |
...
판정 성능은 모델 비교와 같은 방식으로 채점했습니다. 30초 라벨마다 마지막 5초 판정 하나를 보고, 60초 안에 이어진 경보는 한 덩어리로 묶어 헛경보 1회로 셉니다. 실제로 울리는 경보 줄은 5초 판정에 경보 간격 규칙까지 적용한 뒤 휴대폰이 받는 바이트 수를 그대로 센 것입니다.
