...
이마 착용형 PPG 센서에서 얻은 심박 간격(RR)으로 졸음을 판단하는 모델을 만들고, **FPGA에 이식 가능한 형태로 확정**하는 저장소입니다. 학습은 PC에서 오프라인으로 하고 칩에는 추론만 올립니다.
...
전체 경로 중 `<<` 표시 구간이 ML 파트입니다. 회로·PCB·Verilog 구현은 다른 팀원 담당이라 포함하지 않습니다.
...
이 구간을 파이썬으로 먼저 구현·검증한 뒤, 하드웨어 팀에 **고정소수점 파라미터 · 특징 추출 절차 명세 · 테스트 벡터 · IMU 결합 규칙**을 넘깁니다.
