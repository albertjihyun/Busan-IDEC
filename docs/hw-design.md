...
보드는 없다. 완성의 정의는 다음 네 가지다.

1. ~~PPG-DaLiA의 실제 손목 PPG를 테스트벤치로 재생했을 때 RTL 출력이 파이썬 고정소수점 정답지와 비트 단위로 일치한다.~~ 9/24 기준 실제로 한 것: 판정 블록은 MPD-DF 50명 전 블록(75,186) + 경계 사례를 파이썬 정수 정답과 비트 대조(불일치 0), IMU 규칙은 합성 16 시나리오 + PPG-DaLiA 가슴 가속도 S1 92만 샘플 대조(불일치 0). 실제 PPG 파형을 RTL로 재생하는 검증은 준용 쪽(`ppg_soc_top`, 골든 대조·이마 실측)에서 했다.
...
> **9/24 기준 전부 완료.** 이후 진행: 9/23 6단계 IMU 규칙(PR #15), 9/24 5단계 이마 PPG 전이 검증과 창 품질 규칙(PR #18), `infer_top` 재합성(PR #20). 아래 표는 당시 계획과 기록이다.
...
9/30 범위에서 뺀 것: PPG-DaLiA 5단계(여유 시, 대신 WildPPG로 5단계를 했다), AdVitam 외부 검증(데이터도 9/24에 지움), IMU 착용 검증, Vivado 전력 리포트는 준용 통합 후.
