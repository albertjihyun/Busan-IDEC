...
| `accel_x`, `accel_y`, `accel_z` | 16 signed 각각 | ICM-42670-P 원시값 |
...
| `feat_*` | 아래 참조 | 3단계에서 선택된 특징의 내부 표현. 디버그·UART 전송용 |
...
판정 주기는 30초로 확정했고 창 길이는 30초와 60초를 2단계에서 비교해 정한다. 어느 쪽이든 RR 값을 저장하지 않는다. **30초마다 누산기 다섯 개(N, ΣRR, ΣRR², Σd, Σd²)를 닫는다.** 창이 30초면 그 한 벌이 곧 창이고, 60초면 직전 30초 벌과 더한다. 경계를 넘는 연속 차 하나는 직전 벌의 마지막 RR을 한 개만 기억해 처리한다. 이렇게 하면 BRAM이 필요 없고 레지스터 수십 개로 끝난다.
...
| `window_acc` | 30초 누산기 한 벌. 창 60초 선택 시 두 벌 합산 | 구조 확정, 창 길이는 2단계 |
| `classifier` | 3단계에서 선택된 모델. 트리면 부등식 비교와 순회, 선형이면 곱셈기 하나의 순차 MAC | 모델·파라미터는 3~4단계 |
...
9/10~12는 도구 설치와 문제 정의·특징 선정 논의에 썼다. 9/13 기준으로 다시 잡는다.

| 기간 | 작업 |
|---|---|
| 9/13 | 팀 회의. 경계 신호, 블록 분담, PCB 미제작 전제 확정 |
| 9/13~9/14 | AdVitam 전처리 특징 파일로 특징이 두 집단을 가르는지 사전 확인 |
| 9/14~9/16 | 1단계. 피크 검출과 SQI를 파이썬으로 확정. 1024→240 Hz 다운샘플 영향 확인 |
| 9/17~9/18 | 2단계. 특징 후보 15개, 창 30/60초, 라벨, 분포 확인 |
| 9/19~9/21 | 3단계. 모델 비교, 특징 선택, 민감도·헛경보·비용 표 |
| 9/22~9/23 | 4단계. 정수 정답지, 테스트 벡터 |
| 9/19~9/27 | RTL 작성과 시뮬레이션 통과(병행). `rr_counter`, `window_acc`, `combine`는 모델과 무관하므로 먼저 짠다 |
| 9/24~9/25 | 5단계 축소판(PPG-DaLiA만), 6단계 IMU 규칙(문헌값 환산) |
| 9/28 | Vivado 합성·구현, 자원·타이밍·전력 리포트 |
| 9/29~9/30 | 문서 정리, 설계 개요 최종 |
