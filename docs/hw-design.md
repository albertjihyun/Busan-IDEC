...
후보 15개 중 median NN만 이 구조에 안 들어간다. 정렬이 필요해서다. 3단계에서 선택되면 히스토그램 근사 같은 별도 회로를 설계하고, 선택되지 않으면 만들지 않는다. 개인 기준선 대비 특징은 운전 시작 후 첫 N개 창의 누산값을 레지스터에 보관하면 되고, 직전 창 대비 변화는 직전 창 값 한 벌만 보관하면 된다.
