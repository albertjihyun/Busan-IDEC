...
def main():
    ...
    if not vs:
        ...
        return
    lab = labels()
    done = [name(m, w, "") for m, w, suf in vs if not suf]
    P = [
        "# 모델 비교 결과", "",
        f"비교한 모델·창 조합 {len(done)}개: {', '.join(done)}. "
        "'최고세트'는 1-SE 규칙 대신 안쪽 AUC가 가장 높은 특징 세트를 쓴 변형이다.", "",
        "## 읽는 법", "",
    ...
    P += [
        ...
        table_op(vs, lab, OP), "",
        "놓침 방지를 우선한다는 원칙에 따라 주 동작점으로 정했다. 4회/h 문턱은 폴드 간에 안정적이고(규칙 모델 원값 기준 중앙값 +9.8%, 사분위 +9.4~+10.3%), "
        "2회/h는 문턱이 튀는 폴드가 있다. 법정 비교(EU 2021/1341: KSS 8 이상 경보, 평균 민감도 40% 초과, 90% 신뢰구간 하한 20% 초과)는 '깊은 사건' 칸으로 한다. "
        ...
        f"## 3b. 참고 동작점: 헛경보 {OP2:g}회/시간 ({60 / OP2:.0f}분에 한 번)", "",
        table_op(vs, lab, OP2), "",
